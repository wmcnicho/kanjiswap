...
**Playing by keyboard.** `SwapPassage` tracks an *active word* — the one whose choices are open and whose options the keys pick. Hovering a word makes it active; the first key press opens the first unsolved word rather than answering it blind; a correct answer hands off to the next unsolved word (wrapping to anything skipped) after the success flash, while a wrong one stays put. `SwapWord` exposes `choose()` through a ref so a key press runs the same path as a click, and the active word is scrolled into view. Key hints fade in after 3s of hovering one word, and once revealed they stay revealed — that's a `setting_changed` row (`keyHints`), so it survives a reload.
