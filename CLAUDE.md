...
- **漢字 → かな** (`DIRECTION.toReading`) — the kanji is on the page and the reader types its reading. Typing happens in **one field** (`ReadingComposer`), pinned above the passage, showing the word being answered at size; what's typed is mirrored small over that word where its furigana belongs. An input per word turned the line into a row of boxes and the passage stopped reading like text — `TypeWord` holds no field of its own. A correct reading is recognised the moment it's complete — there's nothing to submit, since only one string can be right. `Enter` offers a wrong answer deliberately (that's what counts a miss); `Tab` walks to the next word. A correct answer hands off to the next word **immediately** — no timer, because waiting on an animation is what stops someone going fast. The word's own flash and its settling furigana play out behind the reader, who is already answering the next one. The reward happens where the reader is already looking: the reading just answered lifts off the field itself in green and fades in 420ms, with what it was worth alongside (`(+17) ×4`) for 700ms and the field taking the same tone briefly. Every part of it is absolutely positioned, `aria-hidden`, and `pointer-events: none` — a reward arriving mid-word must never shift the caret or swallow a keystroke of the next one. The rising text is captured when the answer is *offered*, because by the time the score updates the passage has already moved to the next word.
...
**On a phone.** Below `md` the rail becomes a temporary drawer capped at `min(280px, 85vw)`, and a sticky header carries what the hidden rail would otherwise hold — the mark, the name, the passage tally, a live streak, and **Try again** once a passage is done. Every control that a thumb has to hit gets a 44px minimum *on `xs` only*, since the desktop layout is deliberately tighter: the rail's stage rows, passage rows, and direction bars, the ☰ button, and the font picker. The two direction bars sit **side by side** on a phone and stacked on desktop — stacked hairlines 15px tall, one above the other, are three adjacent targets a thumb can't tell apart. `viewport-fit=cover` is what makes `env(safe-area-inset-*)` report real values on a notched phone; without it the padding that clears the home indicator does nothing. Words carry `scroll-margin-top` on `xs` so the sticky header doesn't cover whatever `scrollIntoView` brings up, and stop selecting text under a coarse pointer so repeated tapping doesn't start a selection. `SwapWord` takes a tap to open its choices: MUI only opens a tooltip on touch after a ~700ms long press, which nobody would guess, and tapping the open word again dismisses it. Option cells get 44px minimums on `xs`. Key hints are suppressed on `(pointer: coarse)` — there are no keys to hint at. Heights prefer `dvh` where supported so a phone's address bar doesn't crop the passage, and `ReadingControls` clears the home indicator with `env(safe-area-inset-bottom)`.
